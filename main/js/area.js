...
function changeCity(){
    ...
    if(a == '全国'){
        ...
        $("#city > option").remove();
        $("#city").append('<option value="0" selected>指定してください</option>');
        $("#city").append('<option value="全地域">全地域で検索</option>');
        for( city in area[a] ){
            $("#city").append('<option>' + area[a][city] + "</option>");
        };
    }
}
function postAreaToSearch(){
    var a = getUrlVars();
    var queryString = "";
    if(location.search !== ''){
        var cnt = 0;
        for(key in a){
            if(!isNaN(key) || key === "prefecture" || key === "city") continue;
            if(cnt === 0){
                queryString += '?';
            }else{
                queryString += "&";
    ...
    queryString += "prefecture=" + $("#prefecture").val();
